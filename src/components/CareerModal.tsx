...
const CareerModal = ({ career, open, onOpenChange }: CareerModalProps) => {
  ...
  const ep = expandedType ? riasecProfiles[expandedType] : null;
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent
        className={`rounded-3xl p-5 overflow-hidden gap-0 border-0 bg-card transition-all duration-300 ${
          expandedType ? 'max-w-[700px]' : 'max-w-sm'
        }`}
      >
        <div className="flex items-stretch gap-4">
          {/* Main card - left side */}
          <div className={`rounded-2xl border border-border overflow-hidden bg-card ${expandedType ? 'w-[340px] shrink-0' : 'w-full'}`}>
            {/* Header */}
              ...
              <div className="flex items-center justify-center gap-5">
                {career.relatedSubtypes.map((sub) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`w-12 h-12 rounded-full flex items-center justify-center ${isExpanded ? 'ring-2 ring-offset-2' : ''}`}
                        style={{
                          backgroundColor: subProfile.color,
                          ...(isExpanded ? { ringColor: subProfile.color } : {}),
                        }}
                      >
          ...
          {expandedType && ep && (
            <div className="flex-1 rounded-2xl border border-border overflow-hidden bg-card animate-fade-in-up min-w-[220px]">
